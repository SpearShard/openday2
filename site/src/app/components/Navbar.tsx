          ...
          >
            <div className="flex items-center space-x-6">
              ...
              <div className="flex flex-col">
                ...
                {/* <span className="text-sm text-gray-600">CIT Open Day 2025</span> */}
              </div>
            </div>
          </motion.div>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="group relative">
      <span className="text-gray-600 hover:text-black font-medium transition-colors duration-300">
        {children}
      </span>
      <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-yellow-400 to-yellow-500 transition-all duration-300 group-hover:w-full" />
    </Link>
  );
}
...
function MobileNavLink({ href, onClick, children }: { href: string; onClick: () => void; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="block py-3 px-4 text-gray-600 hover:text-black font-medium transition-colors duration-300 rounded-lg hover:bg-gray-50"
    >
      {children}
    </Link>
  );
